...
module.exports = async ({ getNamedAccounts, deployments, getChainId }) => {
  ...
  if (chainId === "5") { // Goerli
  ...
  } else if (chainId === "84532") { // base_sepolia
    ...
    try {
      ...
      assetTokenAddress = process.env.BASE_SEPOLIA_ASSET_TOKEN_ADDRESS;
      log("AssetToken not found in deployments. Ensure it is deployed before deploying Bonding.");
      if (!assetTokenAddress) {
        throw new Error("Asset token address is not set in the environment variables.");
      }
    }
  } else {
  ...
  if (!assetTokenAddress) {
    if (["5", "80001", "84532"].includes(chainId)) {
      throw new Error("Asset token address is not set in the environment variables and AssetToken deployment failed.");
    } else {
      throw new Error("Asset token address is not set in the environment variables.");
    }
  }
  ...
  const factory = await ethers.getContractAt("FFactory", factoryDeployment.address, deployer);

  const routerDeployment = await deployments.get("FRouter");
  const router = await ethers.getContractAt("FRouter", routerDeployment.address, deployer);

  // Address of the Artist Factory
  const artistFactoryAddress = process.env.ARTIST_FACTORY_ADDRESS;
  if (!artistFactoryAddress) {
    throw new Error("Artist Factory address is not set in the environment variables.");
  }
  ...
  const bonding = await ethers.getContractAt("Bonding", bondingDeployment.address, deployer);
