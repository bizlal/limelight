...
describe("FERC20 Contract", function () {
  ...
  describe("Transactions", function () {
    it("Should transfer tokens between accounts", async function () {
      ...
      await ferc20.connect(addr1).transfer(addr2.address, ethers.utils.parseEther("50"));
    ...
    it("Should fail if sender doesn’t have enough tokens", async function () {
      ...
      await expect(
        ferc20.connect(addr1).transfer(owner.address, ethers.utils.parseEther("1"))
      ).to.be.revertedWith("ERC20: transfer amount exceeds balance");

      // Owner balance shouldn't have changed.
      expect(await ferc20.balanceOf(owner.address)).to.equal(initialOwnerBalance);
    });
    ...
    it("Should allow owner to exclude from maxTx", async function () {
      ...
      await expect(
        ferc20.connect(addr1).transfer(addr2.address, ethers.utils.parseEther("50"))
      ).to.be.revertedWith("Exceeds MaxTx");
  ...
  describe("Burning", function () {
    it("Should allow owner to burn tokens from an account", async function () {
      ...
      const finalOwnerBalance = await ferc20.balanceOf(owner.address);
      expect(finalOwnerBalance).to.equal(initialOwnerBalance.sub(ethers.utils.parseEther("100")));
      expect(await ferc20.totalSupply()).to.equal(initialSupply.sub(ethers.utils.parseEther("100")));
    });

    it("Should not allow non-owner to burn tokens", async function () {
      await expect(
        ferc20.connect(addr1).burnFrom(owner.address, ethers.utils.parseEther("100"))
