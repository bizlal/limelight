...
describe("Bonding Graduation Mechanism", function () {
  let Bonding, bonding, FFactory, ffactory, FRouter, frouter, FERC20, ferc20, IArtistFactoryV3, artistFactory;
  ...
  beforeEach(async function () {
    ...
    ferc20 = await FERC20.deploy("Asset Token", "ASTK", ethers.utils.parseEther("1000000"), 1); // 1% maxTx
  ...
  it("Should graduate the token when reserveA <= gradThreshold", async function () {
    ...
    await token.connect(addr1).approve(bonding.address, ethers.utils.parseEther("100"));

    // Perform sell transactions
    for (let i = 0; i < 10; i++) {
      await bonding.connect(addr1).sell(ethers.utils.parseEther("10"), this.tokenAddress);
    }
