...
describe("Bonding Contract", function () {
  ...
  beforeEach(async function () {
    ...
    ferc20 = await FERC20Factory.deploy("Asset Token", "ASTK", ethers.utils.parseEther("1000000"), 1); // 1% maxTx
  ...
  describe("Initialization", function () {
    it("Should initialize with correct parameters", async function () {
      ...
      expect(await bonding.fee()).to.equal((buyTax * ethers.constants.WeiPerEther) / 1000);
      ...
      expect(await bonding.artistFactory()).to.equal("0x0000000000000000000000000000000000000003");
  ...
  describe("Launching Tokens", function () {
    it("Should launch a new token successfully", async function () {
      ...
      // Verify events
      const launchedEvent = receipt.events.find(event => event.event === "Launched");
      expect(launchedEvent).to.exist;
      expect(launchedEvent.args.token).to.not.equal(ethers.constants.AddressZero);
      expect(launchedEvent.args.pair).to.not.equal(ethers.constants.AddressZero);
