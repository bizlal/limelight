...
import "@openzeppelin/contracts-upgradeable/token/FERC20/ERC20Upgradeable.sol";
import "@openzeppelin/contracts/token/FERC20/utils/SafeERC20.sol";
...
contract ArtistVeToken is IArtistVeToken, ERC20Upgradeable, ERC20Votes {
    ...
    function stake(uint256 amount, address receiver, address delegatee) public {
        require(
            ...
            "Staking is disabled for private artist"
        ...
        IArtistNft registry = IArtistNft(artistNft);
        uint256 limelightId = registry.stakingTokenToLimelightId(address(this));

        require(!registry.isBlacklisted(limelightId), "Artist Blacklisted");

        if (totalSupply() == 0) {
            initialLock = amount;
        }
        
        registry.addValidator(limelightId, delegatee);
    ...
    function setMatureAt(uint256 _matureAt) public {
        ...
        require(
            IAccessControl(artistNft).hasRole(ADMIN_ROLE, _msgSender()),
            "Not admin"
        );
        matureAt = _matureAt;
    }
