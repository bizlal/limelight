...
contract SafeTransferLibTest is DSTestPlus {
    ...
    function testTransferWithNonContract() public {
        SafeTransferLib.safeTransfer(FERC20(address(0xBADBEEF)), address(0xBEEF), 1e18);
    }
    ...
    function testTransferFromWithNonContract() public {
        SafeTransferLib.safeTransferFrom(FERC20(address(0xBADBEEF)), address(0xFEED), address(0xBEEF), 1e18);
    }
    ...
    function testApproveWithNonContract() public {
        SafeTransferLib.safeApprove(FERC20(address(0xBADBEEF)), address(0xBEEF), 1e18);
    }
    ...
    function testTransferWithNonContract(
        ...
        SafeTransferLib.safeTransfer(FERC20(nonContract), to, amount);
    }
    ...
    function testTransferFromWithNonContract(
        ...
        SafeTransferLib.safeTransferFrom(FERC20(nonContract), from, to, amount);
    }
    ...
    function testApproveWithNonContract(
        ...
        SafeTransferLib.safeApprove(FERC20(nonContract), to, amount);
    }
    ...
    function verifySafeTransfer(
        ...
        uint256 preBal = FERC20(token).balanceOf(to);
        SafeTransferLib.safeTransfer(FERC20(address(token)), to, amount);
        uint256 postBal = FERC20(token).balanceOf(to);
    ...
    function verifySafeTransferFrom(
        ...
        uint256 preBal = FERC20(token).balanceOf(to);
        SafeTransferLib.safeTransferFrom(FERC20(token), from, to, amount);
        uint256 postBal = FERC20(token).balanceOf(to);
    ...
    function verifySafeApprove(
        ...
        SafeTransferLib.safeApprove(FERC20(address(token)), to, amount);

        assertEq(FERC20(token).allowance(address(this), to), amount);
    }
    ...
    function forceApprove(
        ...
        uint256 slot = token == address(erc20) ? 4 : 2; // Standard FERC20 name and symbol aren't constant.
        ...
        assertEq(FERC20(token).allowance(from, to), amount, "wrong allowance");
    }
}
