...
async function main() {
  ...
  const CREATOR_ROLE = ethers.utils.keccak256(
  ...
  const hasExecutorRole = await fRouter.hasRole(
    EXECUTOR_ROLE,
    deployer.address
  );
  ...
  const artistNft = await upgrades.deployProxy(ArtistNft, [deployer.address], {
  ...
  console.log(
    'Bonding Fee (from contract):',
    ethers.utils.formatEther(bondingFee),
    'AST'
  );

  const bondingAssetRate = await bonding.assetRate();
  console.log(
    'Bonding Asset Rate (from contract):',
    bondingAssetRate.toString(),
    '\n'
  );
  ...
  if (bondingAssetRate === 0) {
    throw new Error(
      'Bonding Asset Rate is zero, which will cause division by zero.'
    );
  }
  ...
  const assetTokenContract = await ethers.getContractAt(
    'FERC20',
    assetTokenAddress
  );

  // Define purchaseAmount and fee using ethers.js v6 syntax
  const purchaseAmount = ethers.utils.parseEther(
    process.env.PURCHASE_AMOUNT || '200'
  );
  ...
  if (purchaseAmount <= feeAmount) {
    throw new Error(
      `Purchase amount (${ethers.utils.formatEther(
        purchaseAmount
      )} AST) must exceed the fee (${ethers.utils.formatEther(feeAmount)} AST).`
    );
  }

  // Check Bonding Contract Allowance
  const allowance = await assetTokenContract.allowance(
    deployer.address,
    bondingAddress
  );
  console.log(
    'Bonding Contract Allowance:',
  ...
  const allowanceAfterApproval = await assetTokenContract.allowance(
    deployer.address,
    bondingAddress
  );
  console.log(
    'Bonding Contract Allowance after approval:',
    ethers.utils.formatEther(allowanceAfterApproval),
    'AST\n'
  );

  bonding.on('Launched', (token, pair, tokenIndex) => {
    console.log(
      `Launched Event: Token Address=${token}, Pair Address=${pair}, Token Index=${tokenIndex}`
    );
  });
  ...
  const artistTokenContract = await ethers.getContractAt(
    'FERC20',
    artistTokenAddress
  );
  ...
  if (!done) {
    console.log(
      'Token not yet graduated. Try increasing chunk size or number of loops.'
    );
  }
  ...
  // Check if deployer has enough AST
  const deployerASTBalanceForSell = await artistTokenContract.balanceOf(
    deployer.address
  );
  ...
  // Approve Bonding contract to spend AST if not already approved
  const currentSellAllowance = await artistTokenContract.allowance(
    deployer.address,
    fRouterAddress
  );
  if (currentSellAllowance < sellAmountIn) {
    console.log(
      'Approving Bonding contract to spend additional AST for sell...'
    );
    const approveSellTx = await artistTokenContract.approve(
      fRouterAddress,
      sellAmountIn
    );
  ...
  if (network !== 'local' && network !== 'hardhat') {
    ...
    await verifyContract(assetTokenAddress, 'FERC20', [
      'Test Token',
      'AST',
      ethers.parseEther(process.env.INITIAL_SUPPLY || '1000'),
      parseInt(process.env.MAX_TX || '1000', 10),
    ]);
