...
export function useAllowance(TOKEN_ADDRESS, address, LIMELIGHT_ADDRESS) {
  ...
  const userAllowance = data ? BigNumber.from(data.toString()) : BigNumber.from(0);
...
export function useSwapETHForTokens() {
  const { writeContract: swapETHForTokens, isLoading, error } = useWriteContract();
...
export function useSwapTokensForETH() {
  const { writeContract: swapTokensForETH, isLoading, error } = useWriteContract();
